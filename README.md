...
> **說明**：S60 模型 R²=0.360，M60 模型 R²=0.206，兩者皆具有預測能力。S60 表現較佳，預測誤差約 28 kgf/cm²。
...
| 項目 | 選擇 | 原因 |
|:---|:---|:---|
| **演算法** | Lasso Regression | 自動特徵選擇、防止過擬合 |
| **驗證方式** | LOOCV | 充分利用每筆資料 |
| **正則化** | α = 0.1 | 平衡偏差與變異 |
...
> **說明**：預測值與實際值分布於對角線附近，多數樣本落在 ±30 kgf/cm² 範圍內，預測效果良好。
...
> **說明**：M60 的關鍵因素與 S60 不同，坍度呈負向影響（坍度過大會降低強度），矽灰與藥劑則有正向效果。

### 預測效果

![M60 預測散佈圖](M60_prediction_scatter.png)

> **說明**：M60 模型 R²=0.206，預測能力較 S60 略低，部分樣本離散度較大，建議增加測試樣本以提升準確度。
...
| 檔案 | 說明 |
|:---|:---|
| `models/model_s60_v3.pkl` | S60 預測模型 |
| `models/model_m60_v3.pkl` | M60 預測模型 |
| `models/scaler_s60_v3.pkl` | S60 標準化器 |
| `models/scaler_m60_v3.pkl` | M60 標準化器 |
...
| v2 | 2026-02-04 | 修正版：X1~X3 平均強度，完整 17 特徵 |
| **v3** | **2026-02-04** | **單位正確：kgf/cm²，模型重新訓練，加入圖表** |

---

*報告產生時間: 2026-02-04 11:15*
